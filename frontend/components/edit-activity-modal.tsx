...
interface EditActivityModalProps {
  activity: Activity
  onSubmit: (activityId: string, data: ActivityFormData) => void
  onCancel: () => void
}
...
export function EditActivityModal({ activity, onSubmit, onCancel }: EditActivityModalProps) {
  ...
  useEffect(() => {
    if (activity) {
      setFormData({
        ...
        time: activity.time,
        tags: activity.tags,
        repeatFrequency: getRepeatFrequencyFromPattern(activity.repeatPattern),
        customDays: getCustomDaysFromPattern(activity.repeatPattern),
      })
    }
  }, [activity])

  const getRepeatFrequencyFromPattern = (pattern: string): RepeatFrequency => {
  ...
  const getCustomDaysFromPattern = (pattern: string): string[] => {
    if (pattern.startsWith("Every ")) {
      const daysString = pattern.replace("Every ", "")
